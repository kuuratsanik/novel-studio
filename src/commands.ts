...
import { isLocalProvider, resolveLocalUrl } from "./services/localEngines";
...
export async function insert(text: string): Promise<void> {
  ...
  if (!ok) throw new Error("Insert was rejected.");
}
...
export async function replaceSel(text: string): Promise<void> {
  ...
  if (!ok) throw new Error("Replace was rejected.");
}
...
export async function pickRoute(keys: KeyManager): Promise<{ provider: string; model: string; localUrl: string }> {
  ...
  const configuredUrl = cfg.get<string>("localTextUrl") || "http://127.0.0.1:11434";
  const choose = (provider: string, model: string) => ({
    provider,
    model,
    localUrl: resolveLocalUrl(provider, configuredUrl),
  });
  if (forced && forced !== "auto") {
    return choose(forced, defaultModel || (forced === "ollama" ? "llama3.1" : "auto"));
  }
  const hasCloud =
    (await keys.hasKey("openrouter")) ||
    (await keys.hasKey("anthropic")) ||
    (await keys.hasKey("openai")) ||
    (await keys.hasKey("gemini")) ||
    (await keys.hasKey("novelai"));
  if (offline || !hasCloud) {
    return choose("ollama", defaultModel || "llama3.1");
  }
  if (await keys.hasKey("anthropic")) return choose("anthropic", defaultModel || "auto");
  if (await keys.hasKey("openrouter")) return choose("openrouter", defaultModel || "auto");
  if (await keys.hasKey("openai")) return choose("openai", defaultModel || "auto");
  if (await keys.hasKey("gemini")) return choose("gemini", defaultModel || "auto");
  return choose("novelai", defaultModel || "kayra-v1");
}
...
export async function generatePacked(text: TextRouter, keys: KeyManager, prompt: string, systemPrompt: string) {
  ...
  const cfg = vscode.workspace.getConfiguration("novelStudio");
  const privacy = cfg.get<boolean>("privacyLocalCodex") ?? false;
  const timeoutMs = cfg.get<number>("requestTimeoutMs") ?? 120_000;
  const cloud = !isLocalProvider(route.provider);
